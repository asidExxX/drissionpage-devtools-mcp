 ...
 */
import { analyzeWebSocketFramesV2, formatGroupMessages, formatRecentMessages, formatTrafficSummary, formatWebSocketFrameDetail, } from '../formatters/websocketFormatter.js';
import { zod } from '../third_party/index.js';
import { ToolCategory } from './categories.js';
import { defineTool } from './ToolDefinition.js';
const DIRECTION_OPTIONS = ['sent', 'received'];
export const listWebSocketConnections = defineTool({
    ...
    schema: {
        pageSize: zod
            ...
            .positive()
            .optional()
            .describe('Maximum number of connections to return. When omitted, returns all connections.'),
        pageIdx: zod
            .number()
            .int()
            .min(0)
        ...
        urlFilter: zod
            .string()
            .optional()
            .describe('Filter connections by URL. Only connections containing this substring will be returned.'),
        includePreservedConnections: zod
            .boolean()
            .default(false)
            .optional()
            .describe('Set to true to return the preserved connections over the last 3 navigations.'),
    },
...
export const getWebSocketMessages = defineTool({
    ...
    schema: {
        ...
        groupId: zod
            .string()
            .optional()
            .describe('Filter by group ID (A, B, C, ...). Get group IDs from analyze_websocket_messages first.'),
        pageSize: zod
            .number()
            .int()
            .positive()
        ...
        show_content: zod
            ...
            .default(false)
            .optional()
            .describe('Set to true to show full message payload. Default false (summary only) to avoid large binary output.'),
    },
    handler: async (request, response, context) => {
        ...
        if (request.params.groupId) {
            ...
            if (!summary) {
                summary = analyzeWebSocketFramesV2(ws.frames, request.params.wsid, ws.connection.url);
            ...
            if (!indices || indices.length === 0) {
                ...
                response.appendResponseLine(`Available groups: ${summary.groups.map(g => g.id).join(', ')}`);
                return;
            }
        ...
        response.appendResponseLine(`## Recent Messages (wsid=${request.params.wsid})`);
...
export const getWebSocketMessage = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (frameIndex >= ws.frames.length) {
            throw new Error(`Frame index ${frameIndex} out of range. Total frames: ${ws.frames.length}`);
        }
...
export const analyzeWebSocketMessages = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        const summary = analyzeWebSocketFramesV2(frames, request.params.wsid, ws.connection.url);
        context.cacheTrafficSummary(request.params.wsid, summary);
        ...
        response.appendResponseLine(`- View group: \`get_websocket_messages(wsid=${request.params.wsid}, groupId="A")\``);
        response.appendResponseLine(`- View single: \`get_websocket_message(wsid=${request.params.wsid}, frameIndex=0)\``);
    },
});
