...
export const listFrames = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            response.appendResponseLine(`${prefix}${i}: ${frame.url() || '(empty)'}${name}${marker}`);
        }
    },
});
...
export const selectFrame = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (frameIdx >= frames.length) {
            response.appendResponseLine(`Invalid frame index ${frameIdx}. Use list_frames to see available frames (0-${frames.length - 1}).`);
            return;
        }
        ...
        if (frameIdx === 0) {
            ...
            const name = frame.name() ? ` (name: "${frame.name()}")` : '';
            response.appendResponseLine(`Switched to frame ${frameIdx}: ${frame.url()}${name}`);
        }
    },
});
