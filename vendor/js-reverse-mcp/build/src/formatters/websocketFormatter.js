 ...
 */
export function formatWebSocketConnectionShort(ws, id) {
    ...
    const receivedCount = ws.frames.filter(f => f.direction === 'received').length;
    // 检测是否有二进制消息
 ...
 */
export function formatWebSocketConnectionVerbose(ws, id) {
    const lines = [];
    ...
    if (ws.connection.initiator) {
        ...
        if (ws.connection.initiator.stack?.callFrames.length) {
            ...
            for (const frame of ws.connection.initiator.stack.callFrames.slice(0, 5)) {
                lines.push(`  - ${frame.functionName || '(anonymous)'} at ${frame.url}:${frame.lineNumber + 1}:${frame.columnNumber + 1}`);
            }
        }
    }
 ...
 */
export function formatWebSocketFrameDetail(frame, index) {
    const lines = [];
    ...
    lines.push(`### Payload`);
    lines.push(formatPayload(frame.payloadData, frame.opcode, PAYLOAD_SIZE_LIMIT * 2));
    return lines;
}
...
function formatPayload(payload, opcode, sizeLimit = PAYLOAD_SIZE_LIMIT) {
    if (opcode === 2) {
        // Binary data
 ...
 */
function base64ToHex(base64, maxBytes = 4) {
    try {
 ...
 */
function getHead4B(payload, opcode) {
 ...
 */
function guessHintFromHead(head4B) {
    if (!head4B)
        return '-';
    // Gzip magic number: 1f 8b
    if (head4B.startsWith('1f8b'))
        return 'Gzip';
    ...
    if (head4B.startsWith('7b') || head4B.startsWith('5b'))
        return 'JSON';
 ...
 */
export function analyzeWebSocketFramesV2(frames, wsid, url) {
 ...
 */
export function formatTrafficSummary(summary) {
    const lines = [];
    lines.push(`## WebSocket Traffic Summary (wsid=${summary.wsid})`);
    lines.push(`Duration: ${formatDuration(summary.durationMs)} | Total: ${summary.totalFrames} frames (↑${summary.sentCount} sent, ↓${summary.receivedCount} received)`);
    lines.push(``);
    ...
    for (const group of summary.groups) {
        ...
        lines.push(`| ${group.id} | ${dir} | ${group.count} | ${head} | ${sizeRange} | ${group.hint} | ${samples} |`);
    }
    return lines;
}
/**
 ...
 */
export function formatGroupMessages(frames, indices, groupId, options) {
    const lines = [];
    ...
    lines.push(`Base: ${new Date(baseTimestamp).toISOString().split('T')[1].slice(0, 12)}`);
    lines.push(``);
    ...
    if (indices.length > offset + pageSize) {
        lines.push(``);
        lines.push(`Showing ${offset + 1}-${offset + paginatedIndices.length} of ${indices.length}. Use pageIdx=${pageIdx + 1} for more.`);
    }
    return lines;
}
/**
 ...
 */
export function formatRecentMessages(frames, options) {
    const lines = [];
    const pageSize = options?.pageSize ?? 20;
    const pageIdx = options?.pageIdx ?? 0;
    const offset = pageIdx * pageSize;
    const paginatedFrames = frames.slice(offset, offset + pageSize);
    lines.push(`Showing ${offset + 1}-${offset + paginatedFrames.length} of ${frames.length} frames`);
    ...
    for (let i = 0; i < paginatedFrames.length; i++) {
        ...
        lines.push(`| ${frameIdx} | ${dir} | ${timeDelta} | ${size} | ${headStr} |`);
    }
